...
class Dashboard {
    ...
    updatePatientsStats(patients, appointments) {
        ...
        const newTodayElement = document.getElementById('newPatientsToday');
        const withAppointmentsElement = document.getElementById('patientsWithAppointments');
        const inactiveElement = document.getElementById('inactivePatients');

        if (totalElement) totalElement.textContent = totalPatients.toLocaleString();
        if (newTodayElement) newTodayElement.textContent = newPatientsToday.toLocaleString();
        if (withAppointmentsElement) withAppointmentsElement.textContent = patientsWithAppointments.toLocaleString();
        if (inactiveElement) inactiveElement.textContent = inactivePatients.toLocaleString();
    }
    ...
    updatePatientsPagination(totalItems) {
        ...
        const currentPageSizeElement = document.getElementById('currentPatientsPageSize');
    ...
    // عرض نافذة السجل الطبي
    showPatientHistoryModal(medicalHistory) {
        const modalHTML = `
            <div class="patient-history-modal">
                <div class="history-tabs">
                    <button class="tab-btn active" data-tab="appointments">المواعيد</button>
                    <button class="tab-btn" data-tab="prescriptions">الوصفات</button>
                    <button class="tab-btn" data-tab="lab">التحاليل</button>
                    <button class="tab-btn" data-tab="invoices">الفواتير</button>
                </div>
                
                <div class="tab-content">
                    <div class="tab-pane active" id="appointments-tab">
                        ${this.renderAppointmentsHistory(medicalHistory.appointments)}
                    </div>
                    <div class="tab-pane" id="prescriptions-tab">
                        ${this.renderPrescriptionsHistory(medicalHistory.prescriptions)}
                    </div>
                    <div class="tab-pane" id="lab-tab">
                        ${this.renderLabHistory(medicalHistory.labResults)}
                    </div>
                    <div class="tab-pane" id="invoices-tab">
                        ${this.renderInvoicesHistory(medicalHistory.invoices)}
                    </div>
                </div>
            </div>
        `;
        
        this.showCustomModal('السجل الطبي', modalHTML, 'large');
    }

    renderAppointmentsHistory(appointments) {
        if (appointments.length === 0) {
            return '<p class="empty-state">لا توجد مواعيد سابقة</p>';
        }
        ...
        return appointments.map(apt => `
            <div class="history-item">
                ...
                <div class="history-details">
                    <strong>${apt.type || 'كشف عام'}</strong>
    ...
    renderPrescriptionsHistory(prescriptions) {
        if (prescriptions.length === 0) {
            return '<p class="empty-state">لا توجد وصفات سابقة</p>';
        }
        
        return prescriptions.map(pres => `
            <div class="history-item">
                <div class="history-date">${this.formatDate(pres.date)}</div>
                <div class="history-details">
                    <strong>وصفة طبية</strong>
                    <span>${pres.medications?.length || 0} دواء</span>
                </div>
            </div>
        `).join('');
    }

    renderLabHistory(labResults) {
        if (labResults.length === 0) {
            return '<p class="empty-state">لا توجد تحاليل سابقة</p>';
        }
        
        return labResults.map(lab => `
            <div class="history-item">
                <div class="history-date">${this.formatDate(lab.date)}</div>
                <div class="history-details">
                    <strong>${lab.testName || 'تحليل'}</strong>
                    <span class="status-badge status-${lab.status}">${lab.status || 'مكتمل'}</span>
                </div>
            </div>
        `).join('');
    }
    ...
    renderInvoicesHistory(invoices) {
        if (invoices.length === 0) {
            return '<p class="empty-state">لا توجد فواتير سابقة</p>';
        }
        
        return invoices.map(inv => `
            <div class="history-item">
                <div class="history-date">${this.formatDate(inv.invoiceDate)}</div>
                <div class="history-details">
                    <strong>${inv.invoiceNumber || 'فاتورة'}</strong>
                    <span>${inv.totalAmount || 0} ر.س</span>
                    <span class="status-badge status-${inv.status}">${inv.status || 'معلقة'}</span>
                </div>
            </div>
        `).join('');
    }
